...
import { isValidLabLease } from "@/lib/lab/labApproval";
import type { LabLeaseResponse, LabScopeManifestResponse } from "@/lib/lab/types";
      ...
      {lease ? (
        <dl className="mt-2 grid gap-0.5 font-mono text-xs opacity-90 sm:grid-cols-2">
          ...
          <div>
            ...
            <dd className="inline">{String(lease.policy?.requires_approval ?? false)}</dd>
          </div>
        </dl>
      ) : null}
