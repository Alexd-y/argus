...
class TestCallLLM:
    ...
    async def test_returns_content_when_mock_succeeds(self) -> None:
        ...
        with (
            ...
            patch("src.llm.adapters.httpx.AsyncClient") as mock_client_cls,
        ):
    ...
    async def test_no_api_key_leak_in_error_message(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with (
            patch.dict(os.environ, {"OPENAI_API_KEY": secret_key}),
            patch("src.llm.adapters.httpx.AsyncClient") as mock_client_cls,
        ):
            mock_client = AsyncMock()
            mock_client.post = AsyncMock(
            ...
            with (
                ...
                pytest.raises(LLMAllProvidersFailedError) as exc_info,
            ):
                await call_llm("Hi")
