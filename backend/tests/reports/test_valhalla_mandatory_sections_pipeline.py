...
def test_mandatory_sections_populated_when_tool_health_completed() -> None:
    ...
    for section in (
        ...
        assert statuses[section]["status"] not in {
            "not_assessed",
            "not_executed",
            "no_data",
        }
...
def test_headers_finding_requires_security_headers_table() -> None:
    ...
    assert {r["status"] for r in ctx.security_headers_table_rows} <= {
        "present",
        "missing",
    }
...
def test_tech_stack_completed_requires_tech_stack_table() -> None:
    ...
    assert any(
        "Next.js" in row.name or "cloudflare" in row.name.lower() for row in ctx.tech_stack_table
    )
...
def test_no_appendices_wrapper() -> None:
    partial = (
        template_env.report_templates_directory() / "partials" / "valhalla" / "appendices.html.j2"
