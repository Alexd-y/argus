...
from src.reports.valhalla_report_context import (
    ...
    SslTlsAnalysisModel,
    _outdated_from_trivy_dependency_rows,
...
def test_valhalla_removes_appendices() -> None:
    ...
    base = (_valhalla_template_root() / "valhalla_secreport_base.html.j2").read_text(
        encoding="utf-8"
    )
    partial = (
        _valhalla_template_root() / "partials" / "valhalla" / "appendices.html.j2"
...
def test_no_docker_socket_errors_in_customer_report() -> None:
    ...
    assert (
        "docker" not in blob or "[path]" in " ".join(str(x.get("summary") or "") for x in j).lower()
    )
...
def test_tool_health_summary_hides_internal_wrapper_names() -> None:
    rows = build_tool_health_summary_rows(
        tool_run_summaries=[
            (
                "sslscan_scan_glomsoposten_vercel_app_2_testssl_va_web_surface__sscan",
                "completed",
            )
        ],
...
def test_tool_health_summary_respects_mandatory_section_status() -> None:
    ...
    assert "Completed" not in {
        j["tls_assessment"]["state_label"],
        j["port_discovery"]["state_label"],
    }
...
def test_fallback_data_populates_security_headers() -> None:
    ...
    assert "missing" in (
        ctx.security_headers_analysis.summary or ""
    ).lower() or ctx.security_headers_analysis.rows[0].get("value_sample")
...
def test_fallback_data_populates_tech_stack() -> None:
    ...
    assert ctx.tech_stack_structured.web_server or any(
        "openresty" in (r.name or "").lower() for r in ctx.tech_stack_table
    )
...
def test_security_header_finding_has_commands() -> None:
    ...
    assert any(
        str(c.get("command", "")).lower().find("curl") >= 0 for c in cmds if isinstance(c, dict)
    )
...
def test_failed_primary_tool_with_fallback_data_is_partial_not_completed() -> None:
    ...
    tech_health = next(
        row
        for row in ctx.tool_health_summary
        if row.get("capability_id") == "technology_fingerprinting"
    )
...
def _good_finding_dict() -> dict[str, object]:
    ...
    return {
        ...
        "description": "Parameter id is not sanitized. Evidence includes error-based response. "
        + "c" * 12,
        "cvss": 8.1,
        "cvss_score": 8.1,
        "proof_of_concept": {
