...
class TestPlaywrightIntegration:
    ...
    def test_mcp_browser_tools_registered(self):
        ...
        mcp_path = os.path.normpath(
            os.path.join(base, "..", "..", "mcp-server", "tools", "kali_registry.py")
        )
        if os.path.exists(mcp_path):
            with open(mcp_path, encoding="utf-8") as f:
                content = f.read()
            browser_count = content.lower().count("browser_")
            assert browser_count >= 5, (
                f"Expected 5+ browser tool registrations, found {browser_count}"
            )
        else:
...
class TestAuthConfigIntegration:
    ...
    def test_auth_config_importable(self):
        from src.orchestration.auth_config import AuthConfig
...
class TestVulnAgentsIntegration:
    ...
    def test_vuln_agents_importable(self):
        from src.orchestration.vuln_agents import VULN_AGENT_SPECS
    ...
    def test_filter_findings_by_domain(self):
        from src.orchestration.vuln_agents import AgentDomain, filter_findings_by_domain
...
class TestModelTieringIntegration:
    ...
    def test_get_tier_for_task(self):
        from src.llm.task_router import LLMTask, LLMTier, get_tier_for_task
    ...
    def test_get_model_for_tier(self):
        from src.llm.task_router import LLMTier, get_model_for_tier
...
class TestPhaseResumeIntegration:
    ...
    def test_compute_resume_plan(self):
        from src.orchestration.phase_resume import ResumeDecision, compute_resume_plan
    ...
    def test_format_resume_summary(self):
        from src.orchestration.phase_resume import ResumeDecision, format_resume_summary
        from src.orchestration.phases import ScanPhase

        plan = {
            ScanPhase.RECON: ResumeDecision.SKIP,
            ScanPhase.VULN_ANALYSIS: ResumeDecision.RUN_FRESH,
        }
...
class TestTierEscalationIntegration:
    def test_check_tier_escalation_no_escalation(self):
        from src.llm.task_router import LLMTask, check_tier_escalation

        result = check_tier_escalation(LLMTask.ORCHESTRATION, confidence=0.9)
        assert result.escalated is False

    def test_check_tier_escalation_triggered(self):
        from src.llm.task_router import LLMTask, LLMTier, check_tier_escalation
        ...
        ctx = build_source_context(
            [
                CodeSource(
                    file_path="views.py",
                    line_number=10,
                    source_type="flask_request",
                    parameter_name="args",
                    code_snippet="request.args.get('q')",
                )
            ]
        )
...
class TestPoCWatermarkingWiring:
    ...
    def test_stamp_payload(self):
        ...
        stamped = stamp_payload(
            "curl http://target", scan_id="s1", tenant_id="t1", secret_key="test-secret"
        )
...
class TestEpisodicMemoryWiring:
    ...
    def test_store_and_recall(self):
        ...
        entry = EpisodicEntry(
            ...
            cwe="89",
            title="SQL Injection",
            description="Found SQLi in login",
...
class TestDetectionEngineeringWiring:
    ...
    def test_build_detection_prompt(self):
        ...
        sys_prompt, user_prompt = build_detection_prompt([{"title": "SQLi", "cwe": "89"}])
        assert "SQLi" in user_prompt
...
class TestAutoPatchWiring:
    ...
    def test_build_autopatch_prompt(self):
        ...
        sys_prompt, user_prompt = build_autopatch_prompt(
            ...
            file_path="app.py",
            severity="critical",
            vulnerable_code="query=f'SELECT {id}'",
        )
        assert "SQLi" in user_prompt
...
class TestScanEventsWiring:
    ...
    def test_publish_and_subscribe(self):
        ...
        bus.publish(
            ScanEvent(event_type="phase_start", scan_id="s1", tenant_id="t1", phase="recon")
        )
...
class TestPipelineWiringIntegration:
    ...
    def test_adversarial_critic_in_reporting(self):
        ...
        path = os.path.normpath(
            os.path.join(os.path.dirname(__file__), "..", "src", "orchestration", "handlers.py")
        )
        with open(path, encoding="utf-8") as f:
            content = f.read()
    ...
    def test_cost_in_llm_facade(self):
        ...
        path = os.path.normpath(
            os.path.join(os.path.dirname(__file__), "..", "src", "llm", "facade.py")
        )
        with open(path, encoding="utf-8") as f:
            content = f.read()
...
class TestNewWiringP1ToP12:
    ...
    def test_no_silent_episodic_memory_try_except(self):
        ...
        with open(path, encoding="utf-8") as f:
            content = f.read()
        assert (
            "episodic_memory_init_failed" in content or "episodic_memory_recall_failed" in content
        )
...
class TestWiringRound2:
    ...
    def test_confidence_escalation_in_vuln_analysis(self):
        ...
        path = os.path.normpath(
            os.path.join(os.path.dirname(__file__), "..", "src", "orchestration", "ai_prompts.py")
        )
        with open(path, encoding="utf-8") as f:
            content = f.read()
    ...
    def test_sub_agent_spawner_async(self):
        from src.orchestration.sub_agent_spawner import SubAgentSpawner
...
class TestFuzzingExecutionWiring:
    def test_run_fuzzing_campaign_importable(self):
        from src.orchestration.fuzzing import FuzzingRequest, run_fuzzing_campaign
...
class TestRunSymbolicExecutionWiring:
...
class TestModuleIntegrityFixes:
    ...
    def test_adversarial_critic_run_no_executor(self):
        ...
        assert (
            "skipped" in result.overall_assessment.lower()
            or "no" in result.overall_assessment.lower()
        )
