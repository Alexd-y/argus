...
async def test_censys_200_parses_json_search() -> None:
    with (
        patch.dict(os.environ, {"CENSYS_API_KEY": "id", "CENSYS_API_SECRET": "sec"}),
...
async def test_securitytrails_200_parses_json() -> None:
    with (
        patch.dict(os.environ, {"SECURITYTRAILS_API_KEY": "st-key"}),
        patch("src.data_sources.securitytrails_client.httpx.AsyncClient") as mock_cls,
    ):
        mock_client = AsyncMock()
        ...
        mock_cls.return_value.__aenter__ = AsyncMock(return_value=mock_client)
        mock_cls.return_value.__aexit__ = AsyncMock(return_value=None)
        out = await SecurityTrailsClient().query(query_type="domain", domain="example.com")
...
async def test_virustotal_200_parses_json_domain() -> None:
    with (
        patch.dict(os.environ, {"VIRUSTOTAL_API_KEY": "vt"}),
        ...
        mock_cls.return_value.__aenter__ = AsyncMock(return_value=mock_client)
        mock_cls.return_value.__aexit__ = AsyncMock(return_value=None)
        out = await VirusTotalClient().query(query_type="domain", domain="example.com")
    assert out["data"] == payload
    assert out["available"] is True
...
async def test_hibp_unavailable_without_key() -> None:
    with patch.dict(os.environ, {}, clear=True):
        ...
        out = await HIBPClient().query(query_type="breachedaccount", account="user@example.com")
...
async def test_hibp_breaches_200_json_array() -> None:
    with (
        patch.dict(os.environ, {"HIBP_API_KEY": "hibp"}),
        patch("src.data_sources.hibp_client.httpx.AsyncClient") as mock_cls,
    ):
        mock_client = AsyncMock()
        payload = [{"Name": "Adobe"}]
        mock_client.get = AsyncMock(return_value=_ok_response(200, json=payload))
...
async def test_hibp_breachedaccount_404_not_breached() -> None:
    with (
        ...
        mock_cls.return_value.__aenter__ = AsyncMock(return_value=mock_client)
        mock_cls.return_value.__aexit__ = AsyncMock(return_value=None)
        out = await HIBPClient().query(query_type="breachedaccount", account="clean@example.com")
    assert out["data"] == []
