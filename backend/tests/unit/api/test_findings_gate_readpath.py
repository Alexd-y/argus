...
def test_meta_unknown_finding_is_dropped():
    rows = [
        _Row(
            "m1",
            "Unknown finding with insufficient evidence",
            cwe="CWE-200",
            description="The finding is informational with no details provided.",
            severity="info",
        ),
        _Row(
            "m2",
            "Insufficient evidence for unknown informational findings",
            cwe="CWE-200",
            description="Multiple informational findings without sufficient details.",
            severity="info",
        ),
    ]
...
def test_real_finding_with_poc_is_kept():
    rows = [
        _Row(
            ...
            cwe="CWE-79",
            description="Reflected XSS confirmed via dalfox payload.",
            severity="high",
            source_tool="dalfox",
            proof_of_concept={
                "url": "https://t/?q=<x>",
...
def test_thin_llm_and_rich_tool_rows_collapse_via_scan_target():
    ...
    pairs = [
        (
            ...
            "TLS_PROBE finding — https://alleksy.com/",
            "https://alleksy.com/",
        ),
        (
...
def test_multi_host_findings_do_not_over_collapse():
    ...
    rows = [
        _Row(
            ...
            severity="low",
            source_tool="httpx",
            proof_of_concept={"url": "https://a.example/"},
        ),
        _Row(
            "b",
            "Incomplete security HTTP response headers",
            severity="low",
            source_tool="httpx",
            proof_of_concept={"url": "https://b.example/"},
        ),
    ]
