...
class TestCheckScanConcurrency:
    @pytest.mark.asyncio
    async def test_allows_scan_when_below_limit(self, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_rejects_scan_at_limit(self, mock_session: AsyncMock) -> None:
        mock_result = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_rejects_scan_over_limit(self, mock_session: AsyncMock) -> None:
        mock_result = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_allows_two_concurrent_scans(self, mock_session: AsyncMock) -> None:
        mock_result = MagicMock()
    ...
    async def test_fail_open_on_db_error(self, mock_session: AsyncMock) -> None:
        ...
        await check_scan_concurrency(mock_session, "tenant-1", max_concurrent=3, fail_open=True)
    ...
    @pytest.mark.asyncio
    async def test_uses_settings_default_max_concurrent(self, mock_session: AsyncMock) -> None:
        mock_result = MagicMock()
        mock_result.scalar_one.return_value = 0
        mock_session.execute.return_value = mock_result

        with patch("src.policy.scan_concurrency.settings") as mock_settings:
            mock_settings.scan_max_concurrent = 5
            await check_scan_concurrency(mock_session, "tenant-1")

    @pytest.mark.asyncio
    async def test_zero_active_scans_allows_creation(self, mock_session: AsyncMock) -> None:
        mock_result = MagicMock()
        mock_result.scalar_one.return_value = 0
        mock_session.execute.return_value = mock_result
...
class TestScanConcurrencyError:
    def test_error_message_contains_counts(self) -> None:
        exc = ScanConcurrencyError(tenant_id="t-1", active_count=3, max_concurrent=3)
        assert "3" in str(exc)
        assert "3" in str(exc)
        assert "t-1" in str(exc)

    def test_error_attributes(self) -> None:
        exc = ScanConcurrencyError(tenant_id="t-1", active_count=5, max_concurrent=3)
