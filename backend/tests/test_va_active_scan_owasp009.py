...
def _artifact_types_from_sink_raw_text_mock(mock_sink: MagicMock) -> list[str]:
    return [str(c.kwargs.get("artifact_type") or "") for c in mock_sink.call_args_list if c.kwargs]


def _assert_minio_artifact_types_include_dalfox_or_xsstrike(types: list[str]) -> None:
    assert any("tool_dalfox" in at or "tool_xsstrike" in at for at in types), (
...
async def test_va_active_scan_phase_alf_nu_minio_keys_via_upload_raw_artifact_mock() -> None:
    ...
    with (
        patch(
            "src.recon.raw_artifact_sink.upload_raw_artifact",
            MagicMock(return_value="tenant/scan/key"),
        ) as mock_upload,
        patch("src.recon.vulnerability_analysis.active_scan.va_active_scan_phase.settings") as s,
    ):
        s.sandbox_enabled = True
        s.va_active_scan_tool_timeout_sec = 30.0
        s.ffuf_va_wordlist_path = ""
