...
def _detect_wildcard_hints(resolved: dict[str, list[str]]) -> list[str]:
    ...
    for ips, subs in ip_to_subs.items():
        if len(subs) >= 5:
            ...
            if len(ips) > 3:
                ips_str += ", ..."
            hints.append(f"{len(subs)} subdomains share same IP(s): {ips_str} — possible wildcard")
    return hints
...
def build_dns_summary(recon_dir: str | Path) -> str:
    ...
    for row in cname_map:
        val = row.get("value", "")
        for p in _detect_third_party(val):
            third_party.append(f"CNAME: {p} ({row.get('host', '')} -> {val})")
    ...
    if dangling:
        ...
        lines.append("")
        lines.append("*CNAMEs pointing to known takeover-prone targets. Review manually.*")
        lines.append("")
        for d in dangling:
            lines.append(f"- `{d}`")
        lines.append("")
