"""Parser for ``findomain --quiet -t target -u out.txt`` output (ARG-032 batch 4b).

findomain (rust) writes one subdomain per line to ``/out/findomain.txt``::
...
EVIDENCE_SIDECAR_NAME: Final[str] = "findomain_findings.jsonl"
_CANONICAL_NAMES: Final[tuple[str, ...]] = ("findomain.txt", "findomain.log")
...
def parse_findomain(
    ...
    """Translate findomain text output into FindingDTOs."""
    del stderr
    ...
    for raw_line in text.splitlines():
        ...
        if len(keyed) >= _MAX_FINDINGS:
            _logger.warning(
                "findomain.cap_reached",
                extra={
                    "event": "findomain_cap_reached",
                    "tool_id": tool_id,
                    "cap": _MAX_FINDINGS,
                },
            )
