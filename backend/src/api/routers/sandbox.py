...
async def sandbox_python(body: SandboxPythonRequest):
    ...
    interpreter. It is gated behind BOTH ``settings.debug`` AND
    ``ARGUS_SANDBOX_PYTHON_ENABLED`` so that a flag accidentally left enabled in
    a production image (where ``DEBUG`` is false) can never open the endpoint.
    Never enable this in production or expose it to untrusted callers. The regex
    blocklist is a convenience guardrail, not a security boundary.
    """
    if not (settings.debug and settings.argus_sandbox_python_enabled):
        return JSONResponse(
            ...
            content={
                ...
                "detail": (
                    "Sandbox Python execution is disabled. It requires BOTH DEBUG=true "
                    "and ARGUS_SANDBOX_PYTHON_ENABLED=true (local/dev only)."
                ),
            },
        )
