...
* **Planner** → :class:`~src.llm_orchestrator.schemas.loader.ValidationPlanV1`
...
:class:`~src.llm_orchestrator.llm_provider.LLMRequest`, call the provider, and
...
* Provider failures bubble up unchanged
  (:class:`~src.llm_orchestrator.llm_provider.LLMProviderUnavailableError` etc.).
"""
...
def _sanitize_pydantic_errors(exc: ValidationError) -> tuple[str, str]:
    ...
    :mod:`src.llm_orchestrator.schemas.loader` but is duplicated here to keep
