"""CLI utility for the ARGUS Web Workbench extension-manifest signing workflow.

Mirrors :mod:`backend.scripts.prompts_sign` but operates on extension manifest
YAML files (``*.yaml`` under an extensions directory) and the matching
``SIGNATURES`` / ``_keys/`` directory. Signing binds the exact bytes-on-disk
(SHA-256 + Ed25519); the loader in
:mod:`src.web_workbench.extensions.manifest` is fail-closed and refuses any
manifest that is unsigned, signed by an unknown key, or tampered with.

Three argparse-driven sub-commands (CI-friendly):

* ``generate-keys --out <dir>`` — fresh Ed25519 dev keypair (never production).
* ``sign --key <priv> --extensions-dir <dir> --out <SIGNATURES>`` — recompute
  SHA-256 + Ed25519 for every ``*.yaml`` and rewrite the manifest (atomic).
* ``verify --extensions-dir <dir> --signatures <SIGNATURES> --keys-dir <keys>``
  — verify every manifest; exit non-zero on any mismatch. Stack traces are
  NEVER leaked (one structured JSON line per event).
"""
...
# Allow `python backend/scripts/extension_sign.py ...` invocation.
...
_logger = logging.getLogger("extension_sign")
...
def _cmd_generate(out_dir: Path, name: str) -> int:
    ...
    _emit(
        ...
        warning=(
            ...
            "/ k8s Secret. Delete the private key after signing if you do not "
            "plan to rotate again."
        ),
    )
    return 0
...
def _iter_yaml(extensions_dir: Path) -> list[Path]:
    return sorted(p for p in extensions_dir.glob("*.yaml") if p.is_file())


def _cmd_sign(key_path: Path, extensions_dir: Path, out_path: Path) -> int:
    if not key_path.is_file():
        return _fail("sign.error", reason="private key file not found", key_path=str(key_path))
    if not extensions_dir.is_dir():
        return _fail(
            "sign.error",
            reason="extensions dir does not exist",
    ...
    yaml_paths = _iter_yaml(extensions_dir)
    if not yaml_paths:
        return _fail(
            "sign.error",
            reason="no manifest YAML descriptors found",
            extensions_dir=str(extensions_dir),
        )
    ...
    for yaml_path in yaml_paths:
        ...
        rel = yaml_path.relative_to(extensions_dir).as_posix()
...
def _cmd_verify(extensions_dir: Path, signatures_path: Path, keys_dir: Path) -> int:
    if not extensions_dir.is_dir():
        return _fail(
            "verify.error",
            reason="extensions dir does not exist",
    ...
    expected_paths = {p.relative_to(extensions_dir).as_posix() for p in yaml_paths}
    ...
    for yaml_path in yaml_paths:
        ...
        try:
            ...
            _fail("verify.error", reason="failed to read manifest", yaml=rel, os_error=exc.strerror)
            failures.append(rel)
            continue
        try:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="extension_sign",
        description="Manage Ed25519 signatures for ARGUS Web Workbench extension manifests.",
    )
    ...
    sign = sub.add_parser("sign", help="Recompute SIGNATURES for all manifests.")
    sign.add_argument("--key", required=True, type=Path, help="Private key path.")
    sign.add_argument(
        "--extensions-dir", required=True, type=Path, help="Extension manifest YAML directory."
    )
    sign.add_argument("--out", required=True, type=Path, help="SIGNATURES output path.")

    ver = sub.add_parser("verify", help="Verify SIGNATURES against the manifest catalog.")
    ver.add_argument(
        "--extensions-dir", required=True, type=Path, help="Extension manifest YAML directory."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "sign":
        return _cmd_sign(key_path=args.key, extensions_dir=args.extensions_dir, out_path=args.out)
