"""CLI utility for managing the ARGUS payload catalog signing workflow.

Mirrors :mod:`backend.scripts.tools_sign` but operates on
``backend/config/payloads/*.yaml`` and the matching ``SIGNATURES`` /
``_keys/`` directory. Three sub-commands, all argparse-driven so the tool
can be wired into CI:

* ``generate-keys --out backend/config/payloads/_keys/``
...
* ``sign --key <priv> --payloads-dir <dir> --out <SIGNATURES>``
    Recomputes SHA-256 + Ed25519 signature for every ``*.yaml`` under
    ``payloads-dir`` and rewrites ``out`` (atomic, sorted by relative path).

* ``verify --payloads-dir <dir> --signatures <SIGNATURES> --keys-dir <keys>``
    Verifies every YAML under ``payloads-dir`` against the manifest. Exits
...
# Allow `python backend/scripts/payloads_sign.py ...` invocation.
...
_logger = logging.getLogger("payloads_sign")
...
def _iter_yaml(payloads_dir: Path) -> list[Path]:
    return sorted(p for p in payloads_dir.glob("*.yaml") if p.is_file())


def _cmd_sign(key_path: Path, payloads_dir: Path, out_path: Path) -> int:
    ...
    if not payloads_dir.is_dir():
        return _fail(
            "sign.error",
            reason="payloads dir does not exist",
    ...
    yaml_paths = _iter_yaml(payloads_dir)
    if not yaml_paths:
        return _fail(
            "sign.error",
    ...
    for yaml_path in yaml_paths:
        ...
        rel = yaml_path.relative_to(payloads_dir).as_posix()
...
def _cmd_verify(payloads_dir: Path, signatures_path: Path, keys_dir: Path) -> int:
    if not payloads_dir.is_dir():
        return _fail(
            "verify.error",
            reason="payloads dir does not exist",
    ...
    expected_paths = {p.relative_to(payloads_dir).as_posix() for p in yaml_paths}
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="payloads_sign",
        description="Manage Ed25519 signatures for the ARGUS payload catalog.",
    )
    ...
    gen.add_argument(
        "--name", default="dev_signing", help="Private key filename stem."
    )
    ...
    sign.add_argument(
        ...
        required=True,
        type=Path,
        help="Payloads YAML directory.",
    )
    sign.add_argument(
        "--out", required=True, type=Path, help="SIGNATURES output path."
    )
    ...
    ver.add_argument(
        "--signatures", required=True, type=Path, help="SIGNATURES file."
    )
    ver.add_argument(
